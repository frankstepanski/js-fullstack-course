...
### Why Checking `response.ok` is Vital

The `fetch()` API does **not** throw an error when the server returns an HTTP error like 404 or 500. Instead, the promise resolves normally, and it's up to **you** to check if the response was OK:
...
If you skip this, the code inside `.then()` continues even if the server failed. This can break your app silently.

#### ✅ Real Example (with `response.ok` check)
...
#### ❌ Real Example (without `response.ok` check)

```js
fetch("https://jsonplaceholder.typicode.com/posts/invalid")
  .then(response => response.json())  // Skips the status check
  .then(data => {
    // This might crash because the API returned HTML or empty content
    console.log("Data:", data.title);  // 🚨 May cause undefined error
  })
  .catch(error => {
    console.error("Caught error:", error.message);  // May not catch logical API issues
  });
```

Without the `response.ok` check:
- You might try to access properties on undefined/null.
- Your app logic continues with invalid data.
- Users see broken UI or confusing errors in the console.

---

### Example: Safe GET Request with Status Check
