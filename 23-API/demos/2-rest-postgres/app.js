...
The actual server starts in `server.js` where we call:

    app.listen(PORT)

Separating configuration from server startup makes
the application easier to maintain, test, and scale.
*/
...
This is how Express constructs full API paths.
*/
...
This middleware catches errors that occur anywhere
in the request pipeline and returns a consistent
error response.
*/
...
Middleware runs before requests reach your routes.
*/

// Allows requests from different domains (useful for frontend apps)
app.use(cors());
...
This allows the server to read JSON data sent
in the body of HTTP requests.
*/
...
These connect route modules to base URL paths.
...
const swaggerOptions = {
  ...
  apis: ["./routes/*.js"] // Files to scan for Swagger comments
};
...
If any route or controller throws an error,
Express passes it here.
*/
...
server.js will import this file and start the server.
*/
