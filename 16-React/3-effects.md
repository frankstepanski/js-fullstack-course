...
- Side effects are actions that occur outside of rendering, such as fetching data or setting up timers.

- Certain code must run after a component is placed in the DOM, not during rendering.

- Effects allow components to perform asynchronous operations, including API requests using Promises or async functions.

- The dependency array controls when an effect runs: on mount, when specific values change, or not at all.

- React manages effects to avoid issues like duplicate executions, stale data, and unnecessary re-renders.

- Effects provide a predictable way for components to interact with the outside world while keeping UI updates consistent.

## Next Up: Routing

Now that you understand how state and effects work inside a component, the next step is learning custom hooks — a way to extract your logic (including effects) into reusable functions that can be shared across multiple components.